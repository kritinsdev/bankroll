...
async function slotCatalogScraper(url) {
    ...
    while (await page.$('.aAjaxSubmit')) {
        const randomTimeout = getRandomTimeout(3, 7);
        await page.click('.aAjaxSubmit');
        await page.waitForResponse(response => response.url().includes('https://slotcatalog.com/index.php') && response.status() === 200);
        await page.waitForTimeout(randomTimeout);
    }

    const links = await page.$$eval('.slotCardName', (anchors) =>
    ...
    for (const link of links) {
        ...
        await scrapeSlotCatalogSlot(link);
        await sleep(delay);
    }
...
module.exports = slotCatalogScraper;